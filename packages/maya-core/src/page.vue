<template>
  <div class="maya-page" data-maya-page>
    <slot v-if="error" name="maya-page-fallback">
  ...
  computed: {
    slots() {
      return Object.keys({
        ...this.$slots,
        ...this.$scopedSlots
      }).filter(s => !s.startsWith('maya-page-'))
    }
  },
  created() {
    Promise.all([
      ...
      if (import.meta.env.DEV) {
        console.error('[maya-page] error: ', err)
      }
      throw err
    })
  },
