...
<script>
import Vue from 'vue';
import MayaUI from '@yxfe/hydra-ui';
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        ...
        if (this.isCreateable) { delete ruleConfig.ruleNo; }
        const temp = {
    ...
    async queryDetail(ruleNo) {
      ...
      if (this.isXtcRepo) { path += `?poiId=${this.poiId}` }
      try {
    ...
    reloadMetaConfig() {
      this.metaConfig = this.mataConfigMap[
        `${this.currentPoiType}_${this.currentBusinessType}`
      ];
    },
    renderedFn() {
